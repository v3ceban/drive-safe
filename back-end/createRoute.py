...
# Creates api key by calling the Inrix appToken API, does this every
# time the code is run to ensure the key isnt expired
app_id = "y93c10tjpj"
hash_token = "eTkzYzEwdGpwanx1am9DUmJvMzRsNHZOVU1Db0N1U1M3MlNRbGZpdmprSzFQVkhSdm5U"
auth = requests.get(
    "https://api.iq.inrix.com/auth/v1/appToken?appId="
    + app_id
    + "&hashToken="
    + hash_token
)
...
def getRoutes():
    ...
    polygon_url = (
        "https://api.iq.inrix.com/drivetimePolygons?center="
        + str(start_lat)
        + "%7C"
        + str(start_long)
        + "&duration="
        + str(int(time) // int(points))
    )
...
def getRouteInfo():
    ...
    # creates API url
    route_url = (
        "https://api.iq.inrix.com/route?routeId=" + str(route_id) + "&format=json"
    )
    ...
    for i in range(length):
        route_waypoints.append(
            route["result"]["trip"]["wayPoints"][i]["geometry"]["coordinates"]
        )
...
def getHighwayRoutes():
    ...
    for i in range(0, 100, 2):
        ...
        if math.sqrt(
            math.pow(temp_lat - float(start_lat), 2)
            + math.pow(temp_long - float(start_lat), 2)
        ) > math.sqrt(
            math.pow(far_lat - float(start_lat), 2)
            + math.pow(far_long - float(start_lat), 2)
        ):
            far_long = temp_long
            far_lat = temp_lat
    ...
    return jsonify(
        start_lat=start_lat, start_long=start_long, far_lat=far_lat, far_long=far_long
    )
